...
router.post("/register",async(req,res)=>{
    ...
    const {fname,email,password,apassword}=req.body;

    if(!fname || !email || !password || !apassword){
    ...
    try{
       ...
       if(preuser){
        ...
        res.status(422).json({error:"Password and Re-password are not matching"})
       }else{
        const finalUser=new USER({
            fname,email,password,apassword
        });
...
router.post("/login",async(req,res)=>{
    ...
    if(!email || !password){
        res.status(400).json({error:"Fill the login details"})
    };
    try {
